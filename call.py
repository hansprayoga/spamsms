...
def citcall(nomor):
	...
	r.post('https://www.citcall.com/demo/verification.php',headers={'origin': 'https://www.citcall.com','content-type': 'application/x-www-form-urlencoded','user-agent': 'Mozilla/5.0 (Linux; Android 10; M2004J19C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.XMRimage/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-dest': 'document','referer': 'https://www.citcall.com/demo/index.php'},data={'cellNo':nomor,'csrf_token':token})
	call=r.post('https://www.citcall.com/demo/misscallapi.php',headers={'accept': 'application/json, text/javascript, */*; q=0.01','x-requested-with': 'XMLHttpRequest','user-agent': 'Mozilla/5.0 (Linux; Android 10; M2004J19C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.XMR','origin': 'https://www.citcall.com','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://www.citcall.com/demo/verification.php','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'},data={'cid':nomor,'trying':'0','csrf_token':token}).json()
...
if __name__=='__main__':
	try:
		main()
	except IndexError:exit('%s[%s!%s] Use : python2 %s <nomor> <jumlahspam>\n    Example : python2 %s +628(nomor mantan lo!) 3'%(W,R,W,sys.argv[0],sys.argv[0]))
